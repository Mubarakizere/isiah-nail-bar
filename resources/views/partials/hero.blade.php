...
<section class="relative h-[90vh] min-h-[600px] flex items-center justify-center overflow-hidden bg-gray-900"
                ...
                @foreach($heroSlides as $index => $slide)
                    <button @click="goTo({{ $index }})"
                        ...
                        <span class="absolute inset-0 bg-rose-400 rounded-full origin-left transition-transform duration-[6000ms] ease-linear"
                              :class="current === {{ $index }} ? 'scale-x-100' : 'scale-x-0'"
                              x-show="current === {{ $index }}"></span>
                    </button>
                @endforeach
            </div>
        @endif
    ...
    @else
        ...
        <div class="container mx-auto px-4 relative z-10 text-center">
            <div class="max-w-5xl mx-auto space-y-8 animate-fade-in-up">
                ...
                <div class="pt-8 flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <a href="{{ route('booking.step1') }}" class="min-w-[200px] px-8 py-4 bg-white text-gray-900 font-medium rounded-full hover:bg-rose-50 transition-all duration-300 transform hover:-translate-y-1 shadow-[0_0_20px_rgba(255,255,255,0.3)]">
                        Book Appointment
                    </a>
                    <a href="{{ url('/services') }}" class="min-w-[200px] px-8 py-4 bg-transparent border border-white/30 text-white font-medium rounded-full hover:bg-white/10 hover:border-white transition-all duration-300 backdrop-blur-sm">
                        View Services
                    </a>
                </div>
            </div>
    ...
    {{-- Scroll Indicator (only if slides exist) --}}
    @if($heroSlides->count())
