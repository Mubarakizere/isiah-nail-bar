...
<div class="p-6">
    <div class="mb-8">
        ...
        <p class="text-gray-600">Update profile for {{ $teamMember->name }}</p>
    </div>

    <div class="max-w-3xl">
        <div class="bg-white rounded-2xl shadow-md border border-gray-200 p-8">
            <form action="{{ route('admin.team_members.update', $teamMember->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="role" value="{{ old('role', $teamMember->role) }}" required placeholder="e.g. Receptionist, Manager"
                               class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent transition">
                ...
                <div>
                    ...
                    <textarea name="bio" rows="4" 
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent transition">{{ old('bio', $teamMember->bio) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="display_order" value="{{ old('display_order', $teamMember->display_order) }}"
                               class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent transition">
                    ...
                    <div>
                        ...
                        <select name="active" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent transition">
                            <option value="1" {{ old('active', $teamMember->active) == 1 ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('active', $teamMember->active) == 0 ? 'selected' : '' }}>Inactive</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition cursor-pointer bg-gray-50 relative" id="drop-zone">
                        <div class="space-y-1 text-center">
                         ...
                         {{-- Preview Image --}}
                         @if($teamMember->photo)
                            <img id="preview-image" src="{{ Storage::url($teamMember->photo) }}" alt="Preview" class="absolute inset-0 w-full h-full object-cover rounded-lg">
                        @else
